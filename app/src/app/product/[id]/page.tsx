...
async function getProduct(id: string) {
    try {
        ...
        if (error) {
            ...
            // If the error is essentially "not found" (code PGRST116), returns null to show 404
        ...
        console.error("Exception in getProduct:", e);
        return null; // Return null to trigger 404 instead of 500 if data fetch fails
    }
}

export default async function ProductPage({ params }: ProductPageProps) {
    // 1. Resolve params with error boundary
    let id: string;
    try {
        ...
        console.error("[ProductPage] Error resolving params:", e);
        return notFound();
    }

    // 2. Fetch product with error logging
    const product = await getProduct(id);

    if (!product) {
        console.warn(`[ProductPage] Product ${id} not found, redirecting to 404`);
        notFound();
    }
    ...
    try {
        ...
        const message = `Hola, estoy interesado en el arreglo '${product.name}' de $${product.price.toFixed(2)}. ¿Me podrían dar más información?`;
        ...
        console.error("[ProductPage] Rendering Exception:", renderError);
        throw renderError; // Rethrow to Vercel/Next.js so we see the 500, but logs are now captured.
    }
}
